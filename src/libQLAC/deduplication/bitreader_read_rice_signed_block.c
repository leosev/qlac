{
	...
	ucbits = QLAC__BITS_PER_WORD - br->consumed_bits;
	b = br->buffer[cwords] << br->consumed_bits;  /* keep unconsumed bits aligned to left */

	while(val < end) {
		/* read the unary MSBs and end bit */
		x = y = COUNT_ZERO_MSBS2(b);
		if(x == QLAC__BITS_PER_WORD) {
			x = ucbits;
			do {
				...
				x += y;
			} while(y == QLAC__BITS_PER_WORD);
		}
		b <<= y;
		b <<= 1; /* account for stop bit */
		ucbits = (ucbits - x - 1) % QLAC__BITS_PER_WORD;
		msbs = x;
		...
		x = (QLAC__uint32)(b >> (QLAC__BITS_PER_WORD - parameter)); /* parameter < 32, so we can cast to 32-bit uint32_t */
		if(parameter <= ucbits) {
			ucbits -= parameter;
			b <<= parameter;
		} else {
			...
			b = br->buffer[cwords];
			ucbits += QLAC__BITS_PER_WORD - parameter;
			x |= (QLAC__uint32)(b >> ucbits);
			b <<= QLAC__BITS_PER_WORD - ucbits;
		}
		lsbs = x;
...
incomplete_lsbs:
			...
			/* read the binary LSBs */
			if(!QLAC__bitreader_read_raw_uint32(br, &lsbs, parameter - ucbits))
				return false;
			lsbs = x | lsbs;
	...
	if(ucbits == 0 && cwords < words) {
		...
		cwords++;
		ucbits = QLAC__BITS_PER_WORD;
	}

	br->consumed_bits = QLAC__BITS_PER_WORD - ucbits;
	br->consumed_words = cwords;
